...
class lp_Settings:
    ...
    """Class to hold Langmuir probe settings and constants
    ...
    k_eV : Boltzmann constant in eV/K
    m_e : Electron mass in kg
    e : Elementary charge in C
    u : Atomic mass unit in kg
    e0 : Permittivity of free space in F/m
    T_eV : Electron temperature in eV
    A : Atomic mass number of ion species
    m_i : Ion mass in kg
    skiplines_csv : Number of junk lines in output csv file
    skipfooter_csv : Number of junk lines at bottom of output csv file
    press_int : Integer part of pressure in mTorr
    press_dec : Decimal part of pressure in mTorr
    pow_default : Default power in W
    seqno : Sequence number for file naming
    length_str : Length string for file naming
    prober : Probe radius in meters
    probel : Probe length in meters
    Aprob : Probe area in m^2
    fname_fmt : Formatted filename string



    Filename_fmt will likely be different between software programs generating I-V curves. Adjust as needed.
    """
...
def interp_laframboise(V,I,Vp,Vf,Te,ne,settings):
    ...
    mask = np.where((Ie>0))[0]

    V_e = V[mask]-Vp
    I_e = Ie[mask]

    mask2 = np.where(V_e>Vf-Vp)[0]
    V_temp = V_e[mask2]
    I_temp = I_e[mask2]
    ...
    if len(I_temp) > 2:
        ...
        mask3 = np.where(V_e<Vp-Vp)[0]
        V_temp3 = V_temp[mask3]
        I_temp3 = I_temp[mask3]
